...
# +
file_path = "/dss/dsshome1/0A/ge56ron2/Masters_Thesis/results/results_gpu/pf_results_timeseries_strong.csv"
...
# rebuild sequences
B = 95      ###CHANGE TO 5 FOR FIVE BUS ANALYSIS
...
model = Sequential([
    ...
    Reshape((T, B, 2)),                        # (N, T, 95, 2)  <-- matches y_true
])
...
file_path = f"{results_dir}/Model_SG_VMVA_24h_15min_time_seq2seq_strong_D.keras"
...
y_pred_seq = y_pred_last.reshape(-1, 95, 2)  # (N, 95, 2)   ###CHANGE TO y_pred_last.reshape(-1, 5, 2) FOR FIVE BUS ANALYSIS

y_test_last = y_test_seq[:, -1, :]          # (N, 190)
y_test_seq = y_test_last.reshape(-1, 95, 2)  # (N, 95, 2)   ###CHANGE TO y_test_last.reshape(-1, 5, 2) FOR FIVE BUS ANALYSIS

###SAVE MODEL
file_path = f"{results_dir}/Model_SG_VMVA_24h_15min_feat_EAL2_seq2seq.keras"

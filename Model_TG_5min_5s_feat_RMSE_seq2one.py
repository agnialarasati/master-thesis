...
#"/dss/dsshome1/0A/ge56ron2/Masters_Thesis/allbus_results_finalrural2_gennew.csv"
#file_path = "allbus_results_finalrural2.csv"
...
vm_train_scaled = scaler_vm_pu.transform(vm_train).reshape(y_train_raw.shape[0], 95)
...
va_train_scaled = scaler_va.transform(va_train).reshape(y_train_raw.shape[0], 95)
...
# Output: predict 96 buses × 4 values = 384 outputs
model.add(Dense(units=95 * 2, use_bias=True)) #, kernel_initializer='zeros'

# === Compile ===
model.compile(
    optimizer=Adam(learning_rate=0.0002),
    loss= rmse,
    metrics=[tf.keras.metrics.MeanAbsoluteError(),rmse]
)

model.summary()

# === Reshape y before training ===
y_train_flat = y_train_seq.reshape(y_train_seq.shape[0], -1)  # (samples, 384)
y_val_flat   = y_val_seq.reshape(y_val_seq.shape[0], -1)
...
y_pred_flat = model.predict(X_test_seq)  # shape: (samples, 384)
y_pred_seq = y_pred_flat.reshape(-1, 95, 2)  # reshape back to bus×target format
...
y_pred_inv = inverse_transform_predictions(y_pred_seq, scalers=(scaler_vm_pu, scaler_va)) #scaler_vm_pu, scaler_va, scaler_pq

y_true_inv = inverse_transform_predictions(y_test_seq, scalers=(scaler_vm_pu, scaler_va)) #scaler_pq
